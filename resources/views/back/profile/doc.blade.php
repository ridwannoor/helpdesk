            ...
            <div class="modal-body">
                <form action="/vendor/profile/doc/store" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="vendorjenisdoc_id" id="jenis" class="form-control m-bootstrap-select m_selectpicker" required>
...
@foreach (Auth::user('vendor')->vendordoc as $vd)
  <div class="modal fade" id="doc_modal_3{{ $vd->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      aria-hidden="true">
      <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  ...
                  <form action="/vendor/profile/doc/update" method="POST" enctype="multipart/form-data">
                      ...
                      <div class="form-group">
                          ...
                          <div class="alert alert-default alert-dismissible fade show m-alert m-alert--square m-alert--air" role="alert">
                            <span><a href="{{ url('data_file/profile/doc/'.$vd->file) }}">{{ $vd->file }}</a></span>
                          </div>   

                          {{-- <span class="m-form__help m--font-focus">{{ $vd->file }}</span>  --}}
                          @if($errors->has('file'))
                          <small class="error">{{ $errors->first('file') }}</small>
                          @endif
                      </div>
