            ...
            <div class="modal-body">
                <form action="/vendor/profile/bank/store" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <select name="bank_id" id="jenis" class="form-control m-bootstrap-select m_selectpicker" required>
                    ...
                    <div class="form-group">
                        <label for="image" class="form-control-label">*File Upload :</label>
                        <input type="file" name="image" class="form-control" id="image" >
                        <span class="m-form__help m--font-primary">Upload File Format : .pdf | max: 2Mbps</span>
                        @if($errors->has('image'))
...
@foreach (Auth::user('vendor')->vendorbank as $vb)
  ...
  <div class="modal fade" id="bank_modal_3{{ $vb->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
      aria-hidden="true">
      <div class="modal-dialog modal-lg" role="document">
          <div class="modal-content">
              <div class="modal-header">
                  ...
                  <form action="/vendor/profile/bank/update" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="alert alert-default alert-dismissible fade show m-alert m-alert--square m-alert--air" role="alert">
                            <span><a href="{{ url('data_file/bank/'.$vb->image) }}">{{ $vb->image }}</a></span>
                          </div>   
                        {{-- <span class="m-form__help m--font-focus">{{ $vb->image }}</span>  --}}
                        @if($errors->has('image'))
                        <small class="error">{{ $errors->first('image') }}</small>
                        @endif
                    </div>
