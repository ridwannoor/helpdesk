            ...
            <table class="table table-striped- table-bordered table-hover table-checkable" id="m_table_1_wrapper">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $no++ }}</td>
                        <td>{{ $item->kode_syarat }}</td>
                        <td>{{ $item->detail_syarat }}</td>
                        <td><a href="{{ url('data_file/syarat/'.$item->file_syarat) }}" target="__blank">{{ $item->file_syarat }}</a></td>
                        <td width='130px'>
                            {{-- @if ($crud->show > 0)
                            <a href="/syarattender/show/{{ $item->id }}"
                                class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"><i
                                    class="flaticon-visible"></i></a>
                            @endif --}}
                            @if ($crud->edit > 0)
                            <a href="/syarattender/edit/{{ $item->id }}"
                                class="m-portlet__nav-link btn m-btn m-btn--hover-brand m-btn--icon m-btn--icon-only m-btn--pill"><i
                                    class="flaticon-edit"></i></a>
                            @endif
                            @if ($crud->destroy > 0)
                            <a href="/syarattender/destroy/{{ $item->id }}"
